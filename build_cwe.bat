...
jom clean
jom -j6 plClient plUruLauncher plCrashHandler
IF ERRORLEVEL 1 GOTO BUILDFAIL
copy /y bin\plClient.exe ..\staging\plClient.exe
copy /y bin\plUruLauncher.exe ..\staging\plUruLauncher.exe
copy /y bin\plCrashHandler.exe ..\staging\plCrashHandler.exe
...
copy /y bin\UruCrashHandler.exe ..\staging\UruCrashHandler.exe
